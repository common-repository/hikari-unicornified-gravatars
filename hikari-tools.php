...
class HkUniGrav_HkToolsOptions extends HkUniGrav_HkTools{
	...
	public static function print_admin_option($options_item_index,$options_structure,$database_name,$database_values){
		...
		switch ( $options_item['type'] ) {
			...
			<tr> 
				...
				<td>
					<input type="<?php echo $options_item['type']; ?>" size="<?php echo $options_item['options']['size']; ?>" <?php echo $nameid; echo $tx_width; ?> value="<?php echo $database_values[ $options_item['id'] ]; ?>" />
			...
			<tr> 
				<?php echo $th_with_for; ?>
				<td>
					<textarea <?php echo $nameid; echo $ta_width; ?> cols="<?php echo $ta_options['cols']; ?>" rows="<?php echo $ta_options['rows']; ?>"><?php echo $ta_value; ?></textarea>
...
// -------------------------------------
	...
	public function debugRequestParameters(){
		if($this->debug){
			echo '<p>optionsDBValue:</p>';	$this->echoArray($this->optionsDBValue);
			echo '<p>opStructure:</p>';		$this->echoArray($this->opStructure);
			echo '<p>$_REQUEST:</p>';		$this->echoArray($_REQUEST);
		}
	}
	
	public function debugDBValue(){
		if($this->debug){
			echo '<p>get_option(optionsDBName):</p>';	$this->echoArray(get_option($this->optionsDBName));
			echo '<p>optionsDBValue:</p>';				$this->echoArray($this->optionsDBValue);
		}
	}
